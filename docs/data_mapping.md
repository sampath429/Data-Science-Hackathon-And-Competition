This document explains the OAuth 2.0 Client Credentials flow used by the agent to interact with the Salesforce REST API. To meet enterprise security standards, we utilize **Azure Key Vault** to store sensitive client secrets, ensuring they are never exposed in the application code or environment variables.
